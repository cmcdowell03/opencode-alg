...
| M1: execution and learning | Canonical diagnostics, terminal transitions, error propagation, execution-directory capture, byte bounds, versioned redaction, shared live/historical fencing, read-only health, bounded SDK calls, evidence-backed checker prompts, pre-compact live evidence snapshots | The installed V1 SDK cannot express the required all-tool session permission rule; private evolution model calls fail closed and live intake does not enqueue those blocked identities. No live learning-host acceptance |
