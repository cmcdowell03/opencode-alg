...
- Sample PII only from a synthetic or explicitly approved, staged, redacted fixture. Direct sensitive-column references are rejected. `SELECT *` and unqualified columns are rejected; every selected column must be `alias.column`. Never put canaries, credentials, or operational rows in prompts, logs, fixtures, or shared artifacts.
