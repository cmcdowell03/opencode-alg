import type { Checkpoint, ContextReceipt, MemoryOptions } from "./schemas.ts"
...
export function buildContext(store: MemoryStore, index: SkillIndex, checkpoint: Checkpoint, options: MemoryOptions, model: ModelBudget = {}) {
  ...
  const selected: string[] = [], reasons: string[] = []
  let text = "", omitted = 0, blocked = false
  try {
    ...
    if (environment) { selected.push(environment.id); reasons.push(`mandatory environment ${environment.id}`) }
    ...
    omitted = graph.omitted; reasons.push(...graph.reasons)
    ...
    for (const node of graph.nodes) {
      ...
      if (estimateTokens(text + evidence) > budget - 100) { omitted++; continue }
      if (node.kind === "result") resultGuidanceIncluded = true
      text += evidence; selected.push(node.id); reasons.push(`bounded ${node.kind} evidence ${node.id}`)
    }
    const footer = `\nMemory objects omitted: ${omitted}. Expand with alg_memory_read/alg_memory_search.\n`
  ...
  const receipt: ContextReceipt = { schema_version: 1, owner: checkpoint.owner, revision: checkpoint.revision,
    generation: checkpoint.generation, context_hash: hashText(text), selected, omitted, budget,
