...
class RequiredContextError extends Error {}
const requiredFailure = (error: unknown) => new RequiredContextError(error instanceof Error ? error.message : "mandatory context unavailable")
...
export function buildContext(store: MemoryStore, index: SkillIndex, checkpoint: Checkpoint, options: MemoryOptions, model: ModelBudget = {}) {
  ...
  let text = "", blocked = false, requiredUnavailable = false, budgetOmitted = 0
  let selection: WorkingViewSelection | null = null
  try {
    selection = selectWorkingView(store, index, checkpoint, options)
  } catch (error) {
    blocked = true
    requiredUnavailable = error instanceof RequiredContextError
  ...
  if (selection) try {
    try {
      ...
      if (environment) { rendered.push(environment.id); reasons.push(`mandatory environment ${environment.id}`) }
      for (const binding of checkpoint.skills) {
        if (binding.environment !== checkpoint.environment) throw new Error("skill/environment binding mismatch")
        const node = store.node(binding.id, checkpoint.owner)
        if (node.kind !== "skill" || node.payload.key !== binding.key || node.payload.name !== binding.name) throw new Error("invalid skill binding")
    ...
    for (const node of selection.nodes) {
      ...
      if (estimateTokens(text + evidence) > budget - 100) { budgetOmitted++; continue }
      if (node.kind === "result") resultGuidanceIncluded = true
      text += evidence; rendered.push(node.id); reasons.push(`bounded ${node.kind} evidence ${node.id}`)
    }
    const footer = `\nMemory objects omitted: ${selection.omitted + budgetOmitted}. Expand with alg_memory_read/alg_memory_search.\n`
  ...
  if (blocked && selection) budgetOmitted = selection.selected.length
  const receipt: ContextReceipt = { schema_version: 1, owner: checkpoint.owner, revision: checkpoint.revision,
    generation: checkpoint.generation, context_hash: hashText(text), selected: rendered, candidates: selection?.selected ?? [],
    omitted: (selection?.omitted ?? 0) + budgetOmitted, budget_omitted: budgetOmitted, budget,
  ...
  return { text: options.mode === "assist" ? text : "", receipt, blocked, requiredUnavailable }
}
