...
"""
    ...
    def test_mcp_query_name_cancellation_and_responsive_ping(self):
        ...
        try:
            ...
            send("tools/call", 6, {"name": "query", "arguments": {"sql": "SELECT 1 AS safe"}, "_meta": {"progressToken": "host"}})
            extra = responses.get(timeout=10)
            self.assertFalse(extra["result"]["isError"], extra)
            process.stdin.close()
