...
test("learning evidence and the private answer cite one turn and stay separate records", () => {
  const path = project()
  skill(path, "synthetic-revenue")
  const messages = transcript()
  const memory = new SessionMemoryRuntime(path, { mode: "assist", fallbackTokens: 4096 }, undefined, [])
  memory.observe(messages)
  const resultId = memory.current("owner").pins.find((id) => memory.store.node(id, "owner").kind === "result")
  if (!resultId) throw new Error("missing private answer")
  const result = memory.store.node(resultId, "owner")
  const evidence = buildSkillEvidence(messages, "owner", "final", options)
  if (result.kind !== "result") throw new Error("answer record changed kind")
  expect(result.payload.session_id).toBe(evidence.provenance.session_id)
  expect(result.payload.user_message_id).toBe(evidence.provenance.user_message_id)
  expect(result.payload.assistant_message_id).toBe(evidence.provenance.assistant_message_id)
  expect(result.payload.finish).toBe("stop")
  expect(evidence.provenance.finish).toBe(result.payload.finish)
  expect(result.payload.artifact).not.toBe(evidence.evidence_id)
  expect(evidence.kind).toBe("skill_evolution_evidence")
})
